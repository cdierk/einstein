...
void loop() {
    ...
    if (millis() - timeNow >= 500) {
        if (!slavePresent) {                                            // determine if slave joined the I2C bus
            digitalWrite(LED, HIGH);
            Wire.beginTransmission(SLAVE_ADDR);                    // begin call to slave
            ...
            Wire.requestFrom(SLAVE_ADDR,1);                        //request 16 bytes from slave
            rdata_1 = Wire.receive();                               //received byte from slave
